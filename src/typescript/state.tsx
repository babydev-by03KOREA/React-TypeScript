        ...
        <React.Fragment>
            {
                ...
                arr.map((elem, index) => (
                    <CheckBoxComponents key={index}
                                        className={item.includes(elem.data) ? "btn-edge-on" : "btn-edge-off"}
                                        onClick={() => onClick(elem.data)}>
                        <TextSpan>{elem.text}</TextSpan>
                        <Checkbox colorScheme="chakraPrimary" pos="absolute" top="16px" left="168px" isChecked={item.includes(elem.data)}
